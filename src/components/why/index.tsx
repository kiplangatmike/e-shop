  ...
  return (
    <section className="section m-5">
      <div className="items-center">
        <div className="text-center text-[29px] mb-6">
          <h1>Why should you choose us?</h1>
        </div>
        <div className="flex justify-around">
          {why.map((item, index) => (<ul key={index} className="justify-between"><i className='items-center'>{item.icon}</i><li className="text-[#333333] text-[16px] font-semibold">{item.title}</li><li className="text-[13px] text-[]">{item.description}</li></ul>))}
        </div>
      </div>
    </section>
  );
